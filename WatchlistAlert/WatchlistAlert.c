/*----------------------------------------------------------------------------
	Program : WatchlistAlert.c
	Author  : Tom Stevelt
	Date    : 03/26/2019
	Synopsis: Send messages if watchlist alert is triggered.
	Return  : 
...
int main ( int argc, char *argv[] )
{
	...
	{
		case MODE_ALL:
			sprintf ( WhereClause, "(select count(*) from watchlist where Wmember = member.id and Walerttype != '%c') > 0", ALERT_NONE );
			break;
