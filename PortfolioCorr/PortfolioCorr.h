...
TYPE	int		SawBenchmark;
TYPE	char	OutFileName[256];
TYPE	FILE	*fpOutput;
TYPE	long	ReportMember;
TYPE	char	ReportTable;
TYPE	char	*Filename;

#define		MAXSTOCKS		100
#define		MAXDAYS			2000

TYPE	char	TickerArray[MAXSTOCKS][sizeof(xportfolio.xpticker)];

typedef struct
{
	char	Date[12];
	double	Close[MAXSTOCKS];
} ROW_RECORD;

TYPE	ROW_RECORD		DataArray[MAXDAYS];
TYPE	int				DataCount;
TYPE	int				DataIndex;
